...
- Auto-installing MCP launchers (existing binary → npx → uvx → brew → checksum-verified prebuilt download) for kreuzberg, crawlberg, and liter-llm; CLI/MCP wiring for html-to-markdown and tree-sitter-language-pack
- New skills — kreuzberg: `chunking`, `batch-extraction`, `extracting-keywords`; crawlberg: `mapping-urls`, `automating-the-browser`, `serving-the-api`; xberg-enterprise: `versioning-documents`
